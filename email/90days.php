...
function Pool90($db)
{ 
	$statement= $db->prepare("SELECT id as noteID,pool,doc_type,doc_id,is_notify,admin FROM schoolforms.rcm_notification_v2 where is_notify='no'    	 	and pool='90Pool' group by admin");
 	$statement->execute();
	...
	{	
		foreach($data as $d)
		{
			...
			$Ustatement= $db->prepare("SELECT  rcm_notification_v2.id as id ,  rcm_notification_v2.pool,rcm_notification_v2.doc_type,
			rcm_notification_v2.doc_id,rcm_notification_v2.is_notify,rcm_users_v2.id as userID,rcm_users_v2.fname,  rcm_users_v2.lname,rcm_users_v2.email from rcm_users_v2 inner join rcm_notification_v2 on rcm_users_v2.id=rcm_notification_v2.admin where rcm_notification_v2.pool='90Pool' and rcm_notification_v2.admin=:admin order by rcm_notification_v2.is_notify");
...
function sendEmail($emailData,$db)
{
	...
	$mail->isHTML(true);
	$mail->Subject = "90 Days Until Expiration";
 	$mail->AltBody = "Error";
	...
	foreach($user as $u)
	{
		...
		$mail2->Body = "Hi ".$u["fname"].", <br/><br/> You have the following items that are 90 days away from its expiration date. 
		<br/><br/>
		<ol>".$data."</ol>
