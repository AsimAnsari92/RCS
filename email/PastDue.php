...
function Pool90($db)
{ 
	$statement= $db->prepare("SELECT id as noteID,pool,doc_type,doc_id,is_notify,admin FROM schoolforms.rcm_notification_v2 where is_notify='no'    	 	and pool='pastDue' group by admin");
 	$statement->execute();
...
function sendEmail($emailData,$db)
{
	...
	foreach($user as $u)
	{
		...
		$mail2->Body = "Hi ". $u["fname"].", <br/><br/> You have the following items that have passed its renewal date. 
		<br/><br/>
		<ol>".$data."</ol>
